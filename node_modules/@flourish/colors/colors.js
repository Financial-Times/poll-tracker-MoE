...
function hueRotationGenerator(colorSpace, second_channel_name, third_channel_name) {
	return function (init_colors, angle) {
		...
		return function nextColor() {
			...
			return color(colorSpace(h, second_channel, third_channel)).hex();
		};
	};
}
...
function getCategoricalFunction(state, domain) {
	...
	var unique_domain = set(domain).values();
...
function binnedScale(state, domain, values, interpolator) {
	...
	var scale;
	if (state.bin_mode == "fixed") scale = quantize$1().domain([domain_min, domain_max]);
	else if (state.bin_mode == "quantile") scale = quantile$$1().domain(values);
	else scale = threshold$1().domain(thresholds);
...
function getDomain(values) {
	var domain = Array.isArray(values) ? extent$1(values) : [0, 1];
	return domain;
}
...
function getSequentialFunction(state, values) {
	...
	var scale = sequential(interpolate$$1).domain(domain);
	var colorScale = function (value) {
		return !isNaN(value) ? scale(value) : null;
	};
	colorScale.domain = Object.freeze(domain);
...
var INTERPOLATORS$1 = Object.freeze({
...
function getCustomInterpolator$1(state) {
	...
	return piecewise(INTERPOLATOR_GENERATORS$1[color_space], [min, mid, max]);
}
...
function getDivergingFunction(state, values) {
	...
	var interpolate$$1 = function(t) {
		var interpolation = INTERPOLATORS$1[state.diverging_palette] || getCustomInterpolator$1(state);
		if (state.diverging_reverse) return interpolation(1 - t);
		else return interpolation(t);
	};

	var scale = diverging(interpolate$$1).domain(domain);
	var colorScale = function (value) {
		return !isNaN(value) ? scale(value) : null;
	};
