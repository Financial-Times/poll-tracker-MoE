import { getDots, getMoE } from "../parseData";
...
export function getFacetData({
  ...
  return facetNames.map((facetName) => {
    ...
    const parties = state.gridKey
      ? data.Lines.filter((row) => row.facet === facetName)
          .map((d) => d.party)
    ...
    const plotData = parties.map((party) => {
      ...
      const plotLines = linesData.filter((lineRow) => {
      ...
      return {
        ...
        dots: getDots(pollData, party).filter(
          (el) => el.date >= dateExtent[0] && el.date <= dateExtent[1]
        ),
        areas: getMoE(plotLines, party).filter(
          (el) => el.date >= dateExtent[0] && el.date <= dateExtent[1]
        ),
      };
    });
